...
export function validateForm(obj: tValidationForm): returnType {
    ...
    for (let i = 0; i < joints.length; i += 5) {
        if (isNotAnyTypeOfNumber(String(joints[i])) || isNotAnyTypeOfNumber(String(joints[i + 1]))) {
    ...
    for (let i = 0; i < numJointLoads; i++) {
        ...
        for (let i = 0; i < endIndex; i += 4) {
            ...
            if (isNotAnyTypeOfNumber(String(loads[i + 1]))) {
                (valid = false), messages.push("One or more joint x-direction force values are incorrect in the joint load input section");
            }
            if (isNotAnyTypeOfNumber(String(loads[i + 2]))) {
                (valid = false), messages.push("One or more joint x-direction force values are incorrect in the joint load input section");
            }
            if (isNotAnyTypeOfNumber(String(loads[i + 3]))) {
...
function extractProperties(data: string | string[]): number[] {
...
function isNotIntegerNumber(data: number | string): boolean {
...
function isNotAnyNegativeZeroNumber(data: number | string): boolean {
    if (typeof data === "string") {
        if (data === "" || specChar.test(data) || alphaChar.test(data)) {
            return true;
        }
...
function isNotAnyTypeOfNumber(data: string): boolean {
    if (data === "" || specChar.test(data) || alphaChar.test(data)) {
        return true;
    }
    return false;
...
function isNotBetweenZeroAndOne(data: number | string): boolean {
    if (typeof data === "string") {
        if (data === "" || specChar.test(data) || alphaChar.test(data)) {
            return true;
        }
